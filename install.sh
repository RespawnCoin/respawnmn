...
sleep 2
sudo mkdir /root/.respawn
echo "rpcuser=user"`shuf -i 100000-10000000 -n 1` >> /root/.respawn/respawn.conf
echo "rpcpassword=pass"`shuf -i 100000-10000000 -n 1` >> /root/.respawn/respawn.conf
echo "rpcallowip=127.0.0.1" >> /root/.respawn/respawn.conf
echo "listen=1" >> /root/.respawn/respawn.conf
echo "server=1" >> /root/.respawn/respawn.conf
echo "daemon=1" >> /root/.respawn/respawn.conf
echo "maxconnections=250" >> /root/.respawn/respawn.conf
echo "masternode=1" >> /root/.respawn/respawn.conf
echo "masternodeaddr$IP:5600" >> /root/.respawn/respawn.conf
echo "externalip$IP:5600" >> /root/.respawn/respawn.conf
echo "masternodeprivkey=$KEY" >> /root/.respawn/respawn.conf
...
echo ""
echo ""
echo -e "Thank you for installing the Respawn wallet.  Please move onto the ${RED}NEXT${NC} step."
